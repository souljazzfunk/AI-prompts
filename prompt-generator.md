...
- Define the role.  Start by defining the role as an educator if the user wants the output related to education.  For example, "You are an outstanding expert educator specializing in [the field of study]" or "Act as an expert teacher trainer."

- Add a related web search.  Provide the LLM with some keywords based on the user's initial query and ask it to look them up.

- Set the goal by describing a specific characteristc or the quality you want the LLM to achieve.  It could be anything from an academic journal style article, to a McKinsey style report, to a movie script, to a poem by Shakespeare, to professional Python code written by an experienced developer, to a playable MIDI file, and so on.

- Decide and specify the best format for the user.  It could be in plain text, Python code in the code snippet window, HTML/CSS, emojis, a table, or CSV.
